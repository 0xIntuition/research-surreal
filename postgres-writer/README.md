# RabbitMQ to PostgreSQL Sync Pipeline

A high-performance, event-driven synchronization service that consumes blockchain events from RabbitMQ queues (generated by rindexer) and maintains an up-to-date PostgreSQL database with real-time analytics tables.
...
1. **Consumes blockchain events** from RabbitMQ queues in real-time
...
5. **Sends term updates** via Tokio mpsc channels for in-process communication
6. **Runs an analytics worker** that consumes from mpsc channel and maintains triple-level aggregations
...
│  (RabbitMQ)     │
└────────┬────────┘
         │ events → RabbitMQ Exchanges
         ▼
┌─────────────────────────────────────────────────────────┐
│              RabbitMQ Queues                             │
│  postgres.atom_created, postgres.triple_created, etc.   │
└─────────────────┬───────────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────────┐
...
│  ┌──────────────────────────────────────────────────┐   │
│  │  Multi-channel RabbitMQ consumer                │   │
│  │  (one channel per queue, prefetch: 20)          │   │
│  └──────────────────────────────────────────────────┘   │
└─────────────────┬───────────────────────────────────────┘
                  │
                  ▼
...
┌─────────────────────────────────────────────────────────┐
│           Tokio mpsc channel                             │
│  Send term_id via unbounded channel                     │
└─────────────────┬───────────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────────┐
...
2. **RabbitMQConsumer** (`src/consumer/rabbitmq_consumer.rs`)
   - Consumes events from RabbitMQ queues
   - Multi-channel architecture (one channel per queue)
   - Batches messages for efficiency (10ms timeout, non-blocking)
   - Acknowledges processed messages
   - Handles array event data (multiple blockchain events per RabbitMQ message)

3. **PostgresClient** (`src/sync/postgres_client.rs:12`)
   - Manages database connection pool
   - Processes events and triggers cascade
   - Sends term updates via Tokio mpsc channel
...
5. **Analytics Worker** (`src/analytics/worker.rs:15`)
   - Separate thread consuming from mpsc channel
   - Batches term updates (100 per batch, 100ms timeout)
...
1. **Event arrives** from RabbitMQ queue (e.g., `postgres.deposited`)
...
5. **Transaction commits**
6. **Send via mpsc channel** to analytics worker
7. **Analytics worker** receives and processes update
   - Aggregates triple vault data
   - Updates predicate/object aggregates

### Flow for SharePriceChanged Events

1. **Event arrives** from RabbitMQ queue
...
5. **Transaction commits**
6. **Send via mpsc channel** and **analytics worker** processes
...
- RabbitMQ consumer doesn't ACK messages until full success
- Failed cascades automatically retry via RabbitMQ redelivery
- No manual intervention required for transient failures
- Analytics worker uses in-process mpsc channels (no network overhead)

### When Could This Fail?

The system is vulnerable only in extreme edge cases:

1. **Process crash between transactions + RabbitMQ message loss**
   - Event committed, cascade never runs
   - Message lost before RabbitMQ persists it
   - **Mitigation**: RabbitMQ persistence (durable queues), analytics worker backfill
...
- RabbitMQ ACK logic: `src/core/pipeline.rs`
- Retry semantics: RabbitMQ automatic redelivery on nack
- Analytics communication: Tokio mpsc unbounded channel
...
|----------|---------|-------------|
| `RABBITMQ_URL` | `amqp://admin:admin@localhost:5672` | RabbitMQ connection URL |
| `EXCHANGES` | `atom_created,triple_created,...` | Comma-separated list of exchange names |
| `QUEUE_PREFIX` | `postgres` | Queue name prefix (e.g., `postgres.atom_created`) |
| `PREFETCH_COUNT` | `20` | RabbitMQ QoS prefetch count per channel |
...
| Note: Term updates use Tokio mpsc channels (no configuration needed) |
...
EXCHANGES=atom_created,triple_created,deposited,redeemed,share_price_changed
...
docker run -e DATABASE_URL=... -e RABBITMQ_URL=... postgres-writer
```
...
{
  "healthy": true,
  "rabbitmq_consumer_healthy": true,
