...
> **Important**: For a comprehensive catalog of known issues, bugs, and improvements, see [issues.md](./issues.md). This includes critical data integrity concerns and architectural limitations that should be understood before production deployment.
...
1. **Transaction Consistency Risk** (See [issues.md#1](./issues.md#1-transaction-consistency-risk))
   - Event insertion and cascade updates use separate transactions
   - **Impact**: Potential data integrity issues if cascade fails after event commits
   - **Status**: Documented in code (postgres_client.rs:110-113), requires architectural refactoring
   - **Workaround**: Monitor cascade failure metrics and manually reconcile if needed

2. **Health Endpoint Naming Confusion** (See [issues.md#2](./issues.md#2-misleading-health-check-field-names))
...
3. **Non-Graceful Shutdown** (See [issues.md#3](./issues.md#3-graceful-shutdown-not-implemented))
