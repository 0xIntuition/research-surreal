...
use super::atom_created::{handle_atom_created, AtomCreatedEvent};
use super::deposited::{handle_deposited, DepositedEvent};
use super::generic::handle_generic_event;
use super::redeemed::{handle_redeemed, RedeemedEvent};
use super::share_price_changed::{handle_share_price_changed, SharePriceChangedEvent};
use super::triple_created::{handle_triple_created, TripleCreatedEvent};
...
pub async fn process_event(
    ...
    debug!(
        "Processing event '{}' with transaction info: {}",
        event_name,
    ...
    match event_name {
        ...
        "Deposited" => {
            debug!(
                "Processing Deposited event with raw data: {}",
                serde_json::to_string_pretty(&event_data)
                    .unwrap_or_else(|_| "Failed to serialize event_data for logging".to_string())
            );
        ...
        "TripleCreated" => {
            debug!(
                "Processing TripleCreated event with raw data: {}",
                serde_json::to_string_pretty(&event_data)
                    .unwrap_or_else(|_| "Failed to serialize event_data for logging".to_string())
            );
            let event: TripleCreatedEvent =
                serde_json::from_value(event_data.clone()).map_err(|e| {
        ...
        "Redeemed" => {
            debug!(
                "Processing Redeemed event with raw data: {}",
                serde_json::to_string_pretty(&event_data)
                    .unwrap_or_else(|_| "Failed to serialize event_data for logging".to_string())
            );
            let event: RedeemedEvent = serde_json::from_value(event_data.clone()).map_err(|e| {
        ...
        "SharePriceChanged" => {
            debug!(
                "Processing SharePriceChanged event with raw data: {}",
                serde_json::to_string_pretty(&event_data)
                    .unwrap_or_else(|_| "Failed to serialize event_data for logging".to_string())
            );
        ...
        _ => {
            debug!(
                "Processing generic event '{}' with raw data: {}",
                event_name,
                serde_json::to_string_pretty(&event_data)
                    .unwrap_or_else(|_| "Failed to serialize event_data for logging".to_string())
            );
