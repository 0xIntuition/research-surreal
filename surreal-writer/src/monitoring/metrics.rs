...
lazy_static! {
    ...
    static ref SURREAL_HEALTHY_GAUGE: Gauge = Gauge::new(
        "surreal_writer_surreal_healthy",
        "SurrealDB connection health status (1=healthy, 0=unhealthy)"
    ).unwrap();
    static ref UPTIME_GAUGE: Gauge = Gauge::new(
        "surreal_writer_uptime_seconds",
...
impl Metrics {
    pub fn new() -> Self {
        ...
        REGISTRY.register(Box::new(SURREAL_HEALTHY_GAUGE.clone())).ok();
    ...
    pub async fn set_surreal_health(&self, healthy: bool) {
        *self.surreal_healthy.write().await = healthy;
        SURREAL_HEALTHY_GAUGE.set(if healthy { 1.0 } else { 0.0 });
    }
...
mod tests {
    ...
    fn test_prometheus_metrics_format() {
        ...
        assert!(output.contains("surreal_writer_events_processed_total"));
        assert!(output.contains("surreal_writer_events_failed_total"));
        assert!(output.contains("surreal_writer_batches_processed_total"));
        assert!(output.contains("surreal_writer_processing_duration_seconds"));
